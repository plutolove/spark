package org.apache.spark.sql.execution.command;
/**
 * Returns a qualified path object. Method ported from org.apache.hadoop.fs.Path class.
 * <p>
 * param:  defaultUri default uri corresponding to the filesystem provided.
 * param:  workingDir the working directory for the particular child path wd-relative names.
 * param:  path       Path instance based on the path string specified by the user.
 * @return qualified path object
 */
// not preceding
public  class LoadDataCommand$ implements scala.Serializable {
  /**
   * Static reference to the singleton instance of this Scala object.
   */
  public static final LoadDataCommand$ MODULE$ = null;
  public   LoadDataCommand$ ()  { throw new RuntimeException(); }
  // not preceding
    org.apache.hadoop.fs.Path makeQualified (java.net.URI defaultUri, org.apache.hadoop.fs.Path workingDir, org.apache.hadoop.fs.Path path)  { throw new RuntimeException(); }
}
